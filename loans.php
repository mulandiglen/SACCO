...
$logged_in_user = $_SESSION['login_name']; // Assuming the logged-in user's information is stored in the $_SESSION variable
		...
		<div class="card">
			...
			<div class="card-header">
				<large class="card-title">
					...
					<button class="btn btn-primary btn-sm btn-block col-md-2 float-right" type="button" id="new_application"><i class="fa fa-plus"></i> Apply </button>
					<?php endif; ?>	
				</large>
				...
				<table class="table table-bordered" id="loan-list">
					...
					<tbody>
						...
						<?php
							...
							if($_SESSION['login_type'] == 2):
							$qry = $conn->query("SELECT l.*, CONCAT(b.lastname, ', ', b.firstname, ' ', b.middlename) AS name, b.contact_no, b.address 
                    FROM loan_list l 
                    INNER JOIN borrowers b ON b.id = l.borrower_id 
                    WHERE b.tax_id = '$tax_id' 
                    ORDER BY l.id ASC");
							if (!$qry) {
								die('Query Error: ' . $conn->error);
							}
